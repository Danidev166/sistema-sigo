-- Script para crear las tablas faltantes que completan el sistema SIGO al 100%
...
-- Procedimiento para generar reporte consolidado
...
BEGIN
    ...
    SELECT e.*, 
           ...
           COUNT(DISTINCT CASE WHEN a.presente = 0 THEN a.id END) as total_faltas,
    ...
    GROUP BY e.id, e.nombre, e.apellido, e.rut, e.curso, e.especialidad, e.estado, e.fecha_ingreso;
...
-- Vista para alertas prioritarias
...
WHERE a.presente = 0 AND a.fecha >= DATEADD(day, -7, GETDATE())
...
PRINT 'Script completado exitosamente. Todas las tablas y funcionalidades han sido creadas.'; 